...
/// <summary>
/// Populates a scene with NPCs at randomized, NavMesh-valid points within a radius,
/// rejecting points too close to buildings/obstacles. Runs once on <see cref="Start"/>
/// and is also exposed via the Inspector's context menu for manual re-runs in the Editor.
/// </summary>
public class NPCSpawner : MonoBehaviour
{
    ...
    /// <summary>
    /// Spawns up to <see cref="amount"/> NPCs at random valid points on the NavMesh,
    /// giving up after <c>amount * 10</c> failed attempts to avoid an infinite loop
    /// in tightly-packed or mostly-blocked scenes.
    /// </summary>
    ...
    public void SpawnNPCs()
    {
        ...
        while (spawned < amount && attempts < maxAttempts)
        {
            ...
            // Check both ankle- and chest-height for obstacles so NPCs don't spawn
            // inside low walls or under overhangs.
