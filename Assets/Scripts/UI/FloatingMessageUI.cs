...
/// <summary>
/// A world-space message bubble that follows the camera, always facing it at a fixed
/// distance/offset, and fades in/out on <see cref="Show"/>/<see cref="Hide"/>.
/// </summary>
public class FloatingMessageUI : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        // Smoothly ease toward the current show/hide target instead of snapping,
        // so repeated Show/Hide calls don't cause visible flicker.
        if (canvasGroup)
            canvasGroup.alpha = Mathf.Lerp(canvasGroup.alpha, desiredAlpha, Time.deltaTime * fadeSpeed);
    }

    /// <summary>Sets the message text and begins fading the bubble in.</summary>
    ...
    /// <summary>Begins fading the bubble out.</summary>
    public void Hide()
    {
        desiredAlpha = 0f;
