# SerenityShiftPC
A therapeutic PC simulation for treating anxiety, using controlled daily-life environments to guide users through tasks grounded in clinical practice.

**Built with:** Unity (C#)  
**Platforms:** PC
**Status:** Prototype

**Team Devs:**
- Yarin Keshet
- Ori Hemo

**Occupational Therapy Consultants:**
- Nesya Zarbiv
- Ifat Casovitz
- Coral Ben Baruch

## Links

**Web Version(itch.io):**  
https://yarinkash1.itch.io/serenity-shift-pc-final-build-v11

**Windows Version(Downloadable):**  
https://drive.google.com/file/d/1ck8cyvmC8psE4JHW_wNeD4OEhnAAZcQz/view?usp=sharing

## Licenses & Attribution

This project's own source code is licensed under the [MIT License](LICENSE). It is also built on top of Unity and a number of third-party asset packs (3D models, textures, fonts, and audio). Packs with a confirmed open license are included in this repo; packs without a confirmed redistribution-permitting license are excluded from git (kept locally for development only) — see the full breakdown below.

<details>
<summary>Full third-party asset & audio credits (click to expand)</summary>

#### Confirmed open license — included in this repository
| Asset | Source | License |
|---|---|---|
| Ultimate Stylized Nature | Quaternius | CC0 1.0 |
| Noto Sans Symbols font | The Noto Project Authors | SIL Open Font License 1.1 |
| Roboto-Bold font (bundled with Unity's TextMesh Pro package) | Unity Technologies / Google Fonts | Apache License 2.0 |
| Musical note SFX (Do, Re, Mi, Fa, Sol, La, Si) | Freesound.org user [Jaz_the_MAN_2](https://freesound.org/people/Jaz_the_MAN_2/) | CC0 1.0 |

#### Used in the game, not included in this repository

*(present in the local project for development, but not pushed here — see `.gitignore`)*

- Big Oak Tree FREE (ALP Assets) - https://assetstore.unity.com/packages/3d/vegetation/big-oak-tree-free-279431
- MBJ Plant Pack 01 FREE — MadeByJawns - https://madebyjawns.itch.io/plantpack1
- Splash of Color – Unique Photogrammetry Plants - https://assetstore.unity.com/packages/3d/vegetation/plants/splash-of-color-unique-photogrammetry-plants-211137
- Stylized Plants in Pots - https://sketchfab.com/3d-models/stylized-plants-pots-555a3a3de4674e68a8059714c158f2ef
- Town Constructor 3 - https://assetstore.unity.com/packages/3d/environments/urban/town-constructor-3-71070 (purchased asset, single entity license — not redistributed; purchased by [oriah755](https://github.com/oriah755))
- URP Tree Pack
- WoodenParkBench - https://assetstore.unity.com/packages/3d/props/furniture/wooden-park-bench-game-ready-3d-model-348356
- npc_casual_set_00 - https://assetstore.unity.com/packages/3d/characters/humanoids/humans/npc-casual-set-00-326131
- Free American Sedans (Cars pack) — High Matters - https://www.fab.com/listings/b34c4162-44f4-4902-bd6f-489943afa4bd?lang=en
- Anemone_Hybrida_OBJ - https://sketchfab.com/3d-models/anemone-hybrida-honorine-jobert-d40b58e0af514977aa52b839659dc1d3
- Emilulz Assets
- Flowers And Garden Assets - https://assetstore.unity.com/packages/3d/vegetation/flowers/flowers-and-garden-assets-241214
- Urban Fences - https://assetstore.unity.com/packages/3d/environments/urban/urban-fences-327500 (purchased asset, single entity license — not redistributed; purchased by [oriah755](https://github.com/oriah755))
- LowlyPoly
- NOT_Lonely
- Ornamental Flower Set - https://assetstore.unity.com/packages/3d/vegetation/flowers/ornamental-flower-set-11920
- Parks And Nature Pack - https://assetstore.unity.com/packages/3d/props/parks-and-nature-pack-lite-77362
- Rocks and Vegetation Pack - https://assetstore.unity.com/packages/3d/environments/rocks-and-vegetation-pack-urp-243519
- Trees_WorldSpace_FREE
- Ultrasonic
- Easter Island statue
- Garden statue
- Grass pack
- Guitar player character
- Sleeping angel statue
- Birds pack
- BaseBall ProKit
- Polytope Studio (Lowpoly environments/village)
- Loose FBX character models
- chime_cropped.wav
- City ambience track (easy/medium/hard variants)
- Guitar and Spanish guitar music tracks
- Park/nature ambience recordings
- Bird/NPC compass target icons
- Music-note puzzle icon
- Sketchy arrow graphic
- Keyboard key icons (A/D/S/W/Tab/arrows)
- Home, mouse, and volume icons
- Human Basic Motions 2.4 FREE (NPC animations) — Kevin Iglesias ([keviniglesias.com](https://www.keviniglesias.com/)) — Standard Asset Store EULA: royalty-free, commercial use allowed, attribution not required, resale/redistribution not permitted

</details>

